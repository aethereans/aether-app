        ...
        <div class="status-card">
          <div class="card-header">
            ...
            <div class="info-marker-container" v-show="loadComplete">
              <a-info-marker
                header="Front end is the part that makes this app meaningful to humans."
                text="<p>It generates a graph of entities and relationships from raw network data, and converts the content you create into raw blocks that other users can get from you, read, and interpret. </p><p>Since these users are also nodes, they will forward the content that you generated to the nodes that connect to them as well.</p>"
              ></a-info-marker>
            </div>
            <div class="flex-spacer"></div>
            <a-guidelight
              :notifyState="dotStates.frontendDotState"
            ></a-guidelight>
          </div>
          <div class="card-body">
            <div class="sub-block">
              <div class="sub-header">
                <div class="sub-header-text">Content</div>
                <div class="flex-spacer"></div>
                <a-guidelight
                  :notifyState="dotStates.refresherDotState"
                ></a-guidelight>
              </div>
              <div class="sub-body">
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderTimestamp(fas.lastrefreshtimestamp) }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="sub-block">
              <div class="sub-header">
                <div class="sub-header-text">Graph compiler</div>

                <div class="info-marker-container" v-show="loadComplete">
                  <a-info-marker
                    header="Graph compiler processes raw data into human readable relationships."
                    text="<p>This part of Aether is tasked with converting raw Mim entities to what is visible as communities, threads, posts, signals such as upvotes, downvotes, and so on. </p><p>It receives the incremental updates that the backend has gathered from the network, and then inserts it into its existing network of meaningful relationships. </p><p>This component runs in intervals.</p>"
                  ></a-info-marker>
                </div>
                <div class="flex-spacer"></div>
                <a-guidelight
              ...
              <div class="sub-body">
                ...
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderDuration(fas.lastrefreshdurationseconds) }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="sub-block">
              <div class="sub-header">
                <div class="sub-header-text">Entities in progress</div>

                <div class="info-marker-container" v-show="loadComplete">
                  <a-info-marker
                    header="In-progress entities are those that you create or update while you're using the app."
                    text="
                ...
                <div class="flex-spacer"></div>
                <a-guidelight
                  :notifyState="dotStates.inflightsDotState"
                ></a-guidelight>
              </div>
                      ...
                      <tr>
                        ...
                        <td>
                          <div class="progress-bar-container">
                            <a-progress-bar
                              :percent="inflight.ProgressPercentage"
                            ></a-progress-bar>
                          </div>
                        </td>
                      </tr>
      ...
      <div class="card-block">
        <div class="status-card">
          <div class="card-header">
            ...
            <div class="info-marker-container" v-show="loadComplete">
              <a-info-marker
                header="The back end is the node instance running on your machine that talks to other nodes, and to your front end."
                text="<p>It handles the distribution, validation, maintenance, and propagation of raw Mim content and signals.</p><p>There is a backend running on your device alongside a front end. This is the default way the client works, but it is not the only one. </p><p> For example, some users prefer to run their backends on a cloud service, and have the Aether app connect that backend. The benefit of doing this is that it makes your node always online, and always up to date, even when your computer is shut down. </p><p>It also comes with an additional benefit of being able to service multiple frontends from the same backend, so you can run a node for your family, or as a public service. Back ends are zero-knowledge, and they know nothing about the users present in front ends connected to them. </p>"
              ></a-info-marker>
            </div>
            <div class="flex-spacer"></div>
            <a-guidelight
              :notifyState="dotStates.backendDotState"
            ></a-guidelight>
          </div>
          <div class="card-body">
            <div class="sub-block">
              <div class="sub-header">
                <div class="sub-header-text">Network</div>
                <div class="info-marker-container" v-show="loadComplete">
                  <a-info-marker
                    header="The content in this app comes from a network of nodes like you."
                    text="<p>Your computer connects to these nodes in certain patterns to maximise its efficiency.</p><p>In their ideal state, all nodes in the network retain all of the information on the network, bracketed by time.</p><p> Network connections are pull-only. This means your node cannot push data out into another node, that node needs to ask first. As a result, it's important for other nodes to be able to connect to you, because if they cannot, content you create will not leave your device and it will not become available on the network.</p><p>Aether will attempt to reverse open connections in the case that no one connects to your computer for a while. In this case, your device will ask other people to connect to it, and hand over a live TCP socket to do so.</p><p>Outbound requests are triggered in intervals.</p>"
                  ></a-info-marker>
                </div>
                <div class="flex-spacer"></div>
                <a-guidelight
                  :notifyState="dotStates.networkDotState"
                ></a-guidelight>
              </div>
              <div class="sub-body">
                <div class="row">
                  <div class="row-text">Inbound conns. in last 15m</div>
                  <div class="flex-spacer"></div>
                ...
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderTimestamp(bas.lastinboundconntimestamp) }}
                  </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderTimestamp(bas.lastoutboundconntimestamp) }}
                  </div>
                </div>
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderDuration(bas.lastoutbounddurationseconds) }}
                  </div>
                </div>
                <div class="spacer-row"></div>
                <!-- <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="info-marker-container">
                    <a-info-marker
                      header="Port mapping status indicates whether your router accepted a port open request from Aether."
                      text="
            ...
            <div class="sub-block">
              ...
              <div class="sub-body">
                ...
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderTimestamp(bas.lastdbinserttimestamp) }}
                  </div>
                </div>
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderDuration(bas.lastinsertdurationseconds) }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="sub-block">
              ...
              <div class="sub-body">
                ...
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderTimestamp(bas.lastcachegenerationtimestamp) }}
                  </div>
                </div>
                <div class="row">
                  ...
                  <div class="flex-spacer"></div>
                  <div class="row-data">
                    {{ renderDuration(bas.lastcachegenerationdurationseconds) }}
                  </div>
                </div>
              </div>
            </div>
    ...
    bas(this: any) {
      ...
      Object.keys(bas).forEach(function (key) {
        if (
          bas[key] === 0 ||
          bas[key] === '' ||
          globalMethods.IsUndefined(bas[key])
        ) {
          // The ones below are excluded from labeled 'unknown' and will show zeroes.
          if (
            key === 'dbsizemb' ||
            key === 'maxdbsizemb' ||
            key === 'lastinsertdurationseconds' ||
            key === 'lastcachegenerationdurationseconds' ||
            key === 'inboundscount15' ||
            key === 'outboundscount15'
          ) {
            return
          }
          bas[key] = 'Unknown'
    ...
    fas(this: any) {
      ...
      Object.keys(fas).forEach(function (key) {
        if (
          fas[key] === 0 ||
          fas[key] === '' ||
          globalMethods.IsUndefined(fas[key])
        ) {
          // The ones below are excluded from labeled 'unknown' and will show zeroes.
