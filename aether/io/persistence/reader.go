...
func Read(
	...
	switch entityType {
	case "boards":
		entities, err := ReadBoards(fingerprints, sanitisedBeginTimestamp, sanitisedEndTimestamp, opts.AllProvables_Owner, opts.AllProvables_Limit, opts.AllProvables_Offset)
		if err != nil {
			return result, err
		}
	...
	case "keys":
		entities, err := ReadKeys(fingerprints, sanitisedBeginTimestamp, sanitisedEndTimestamp, opts.AllProvables_Owner, opts.AllProvables_Limit, opts.AllProvables_Offset)
		if err != nil {
			return result, err
		}
...
func ReadBoards(
	...
	var arr []api.Board
	dbArr, err := ReadDbBoards(fingerprints, beginTimestamp, endTimestamp, ownerfp, limit, offset)
	if err != nil {
		return arr, err
	}
	for _, entity := range dbArr {
...
func ReadDbBoards(
	...
	beginTimestamp api.Timestamp,
	endTimestamp api.Timestamp,
	ownerfp string, limit, offset int) ([]DbBoard, error) {
	var dbArr []DbBoard
	var query string
	var args []interface{}
	var err error
	switch reqtype(reqtypeOpts{
...
func ReadKeys(
	...
	var arr []api.Key
	dbArr, err := ReadDbKeys(fingerprints, beginTimestamp, endTimestamp, ownerfp, limit, offset)
	if err != nil {
		return arr, err
	}
	for _, entity := range dbArr {
