...
function main() {
    ...
    fs.readdir(serverSourceDirectory, function(err, files) {
        for (let val of files) {
            let slc = val.split('+')
            moveFiles(val, slc, serverFileEndings, serverSourceDirectory, targetDirectory)
        }
    })
}
    ...
    for (let ending of mapping) {
        if (parsedName[parsedName.length - 1].endsWith(ending[0])) {
            ...
            if (ending[1] === 'linux-server') {
                version = parsedName[0].replace('aether-backend-linux-x64-extverify', '')
            ...
            let destPath = targetDir + '/' + version + '/' + build + '/' + operatingSystem
            // make paths
            mkDirByPathSync(destPath)
            fs.copyFileSync(sourcePath + '/' + unparsedName, destPath + '/' + unparsedName)
