    ...
    <div class="flex-carrier">
      <div class="board-name-container">
        <div class="board-name-image">
          <a-hashimage :hash="$store.state.currentBoard.fingerprint" height="48px"></a-hashimage>
        </div>
    ...
    <div class="flex-carrier">
      <div class="board-actions-container" v-show="!newThreadButtonHidden">
        ...
        <template v-if="!localUserReadOnly">
          <router-link :to="'/board/'+$store.state.currentBoard.fingerprint+'/newthread'" class="button is-primary is-outlined" hasTooltip title="Create a new thread">NEW THREAD</router-link>
        </template>
...
<script lang="ts">
  ...
  export default {
    ...
    methods: {
      ...Vuex.mapActions(['subToBoard', 'unsubFromBoard', 'silenceBoard', 'unsilenceBoard']),
...
<style lang="scss" scoped>
  ...
  .board-name-container {
    ...
    .board-name-text {
      ...
      max-width: 400px;
      height: 36px;
      @include lineClamp(1) // causes text shift between clamped and nonclamped lines. see why. todo
    }
  }
