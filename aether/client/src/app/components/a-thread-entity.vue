<template>
  <router-link class="thread-entity" :to="linkToThread" @click.self :class="{'inflight': isInflightEntity}">
    ...
    <div class="main-data-container">
      <div class="thread-main-data-centerer">
        ...
        <div class="thread-link" v-show="sanitisedLink.length > 0">
          <a :href="sanitisedLink" :title="sanitisedLink" @click.stop>{{sanitisedLink}}</a>
        </div>
        ...
        <div class="thread-metadata">
          <div class="thread-metadata-container">
           <!-- &nbsp;—&nbsp; -->
           <a-boardname :name="thread.viewmetaBoardname" :fingerprint="thread.board"></a-boardname>
            <a-timestamp class="thread-entity-timestamp" :creation="thread.creation" :lastupdate="thread.lastupdate"></a-timestamp>
          </div>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    computed: {
      ...
      voteCount(this: any) {
        ...
        return this.thread.compiledcontentsignals.upvotes - this.thread.compiledcontentsignals.downvotes
      },
      ...
      linkToThread(this: any) {
        ...
        return '/board/' + this.thread.board + '/thread/' + this.thread.fingerprint
      },
      /*----------  Visibility  ----------*/
