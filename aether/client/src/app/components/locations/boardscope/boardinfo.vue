    ...
    <div class="board-info">
        ...
        <div class="user-data-field">
          <template v-if="!editPaneOpen">
            ...
            <div class="actions-rack" v-if="actionsVisible">
              <a class="action edit" v-if="$store.state.currentBoard.selfcreated" @click="toggleEditPane">
      ...
      <div class="user-data-field-container">
        ...
        <div class="user-data-field fingerprint">
          ...
          <a-fingerprint :fp="$store.state.currentBoard.fingerprint"></a-fingerprint>
        </div>
      </div>
    </div>
  </div>
...
<script lang="ts">
  ...
  export default {
    ...
    methods: {
      timeString(input: any) { // All int64 values are provided to JS as strings by gRPC.. which means our timestamps are strings.
        if (parseInt(input) === 0) {
          return 'Never'
        }
      ...
      submitEditExistingBoard(this: any, fields: any) {
        ...
        let vm = this
        fe.SendBoardContent(this.$store.state.currentBoard.fingerprint, board, function(resp: any) {
          vm.toggleEditPane()
    ...
    computed: {
      ...
      inflightEdits(this: any) {
        ...
        for (let val of this.$store.state.ambientStatus.inflights.boardsList) {
          if (this.$store.state.currentBoard.fingerprint !== val.entity.provable.fingerprint) {
            continue
          }
