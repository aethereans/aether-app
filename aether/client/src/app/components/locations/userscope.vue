<template>
  <div class="location user" v-if="$store.state.currentUserLoadComplete">
    ...
    <template v-else>
      <div class="user-side">
        <a-avatar-block :user="$store.state.currentUserEntity"></a-avatar-block>
        <div class="user-info-flex-carrier">
          <div class="user-info-flex-child">
            <div class="user-info">
              <a-markdown :content="$store.state.currentUserEntity.info"></a-markdown>
            </div>
          </div>
        </div>
...
<style lang="scss" scoped>
  ...
  $user-sidebar-base: $mid-base * 0.9;
  .location {
    display: flex;
    flex-direction: row;
    padding-bottom: 0;
  ...
  .user-side {
    ...
    .user-info {
      ...
      &:after {
        ...
        width: 70%;
        height: 1.3em;
        background: linear-gradient(to right, $a-transparent, $user-sidebar-base 75%);
      }
    }
  }
