    ...
    <div class="user-boards">
      ...
      <template v-else>
        ...
        <div v-for="board in boardsList" :key="board.Fingerprint">
          <a-board-listitem :board="board" uncompiled="true"></a-board-listitem>
          <div class="divider"></div>
        </div>
        ...
        <a-no-content
          no-content-text="No communities created in retained history."
          quoteDisabled="true"
          v-if="boardsList.length === 0 && inflightCreates.length === 0"
        >
        </a-no-content>
