<template>
  <div class="composer">
      ...
      <div class="composer-block" :class="{'preview-is-visible':block._previewIsVisible, 'single-row': block.heightRows === 1}">
        <div class="composer-top-rack">
          <div class="errors-container" v-show="!block._previewIsVisible">
            <div class="error" v-show="block._touched &&  block.content.length === 0 && !block.optional && !block.emptyWarningDisabled">
              <div class="error-text">
                You need something in here
              </div>
            </div>
            <div class="error" v-show="block._touched && block.maxCharCount < block.content.length">
              <div class="error-text">
                This is way too long
              </div>
            </div>
            <div class="error" v-show="block._touched && block.minCharCount > block.content.length && block.content.length !== 0">
              <div class="error-text">
                This is too short
              </div>
            </div>
        ...
        <template v-if="block.heightRows === 1 && block.passwordField">
          <div class="single-row-text-container">
            <input type="password" v-show="!block._previewIsVisible" :ref="'textarea_'+index" class="composer-text-entry" v-model.trim="block.content" @keydown="textareaKeydown(index, $event)" @input="textareaInput(index)"></textarea>
          </div>
        </template>
        ...
        <template v-if="block.heightRows === 1 && !block.passwordField">
          <div class="single-row-text-container">
            ...
            <textarea v-show="!block._previewIsVisible" :ref="'textarea_'+index" class="composer-text-entry" v-model.trim="block.content" @keydown="textareaKeydown(index, $event)" @input="textareaInput(index)" :rows="block.heightRows" :placeholder="block.placeholder"></textarea>
        ...
        <div class="composer-bottom-rack" v-if="!(baseData.fields.length>1)">
          <div class="flex-spacer"></div>
          <div class="actions">
            <a class="button is-outlined is-small is-white" v-show="baseData.cancelActionName" @click="maybeCancel">{{baseData.cancelActionName}}</a>
    ...
    <div class="actions-rack" v-if="baseData.fields.length>1">
      <div class="flex-spacer"></div>
      <div class="actions">
        <a class="button is-outlined is-white" v-show="baseData.cancelActionName" @click="maybeCancel">{{baseData.cancelActionName}}</a>
      ...
      */
      textareaInput(this: any, index: any) {
        if (this.baseData.fields[index].spaceDisabled) {
          this.baseData.fields[index].content = this.baseData.fields[index].content.replace(/\s/g, "")
        }
      },
      ...
      saveDraft(this:any) {
        ...
        if (typeof this.baseData === 'undefined' || typeof this.baseData.draft == 'undefined') {
          return
        }
        let dr:any = {}
