    ...
    computed: {
      ...
      */
      ...
      postOwner(this: any) {
        if (this.isInflightEntity) {
            if (this.$store.state.localUserArrived && this.$store.state.localUserExists) {
      ...
      */
      ...
      externalUrl(this:any) {
        return 'aether:/' + this.$store.state.route.path + '?focusSelector=' + this.post.fingerprint + '&highlightSelectors=["' + this.post.fingerprint + '"]'
      },
      uncompiledDeepLink(this:any) {
        return '/board/' + this.post.board + '/thread/' + this.post.thread + '?focusSelector=' + this.post.provable.fingerprint + '&highlightSelectors=["' + this.post.provable.fingerprint + '"]'
      }
    },
    mounted(this: any) {
